...
sql = "create table DATA_MCDS as select p.fips, p.stname,p.ctyname, p.abbrev,p.popestimate2016, case when o.obesity_per is null or o.obesity_per = 0 then 0 else o.obesity_n/(o.obesity_per/100) end as obesity_pop,o.obesity_n, o.obesity_per, case when o.diab_per is null or o.diab_per = 0 then 0 else o.diab_n/(o.diab_per/100) end as diab_pop,o.diab_n, o.diab_per, f.pop2010, f.ohu2010, f.medianfamilyincome, f.lapop20, f.lalowi20, f.lasnap20, f.tractlowi, f.tractsnap, f.lapop20_share, f.lalowi20_share, f.lasnap20_share, m.mccount,case when m.mccount is null or p.popestimate2016 = 0 then 0 else m.mccount/p.popestimate2016 end as mcd_per_cap, 	case when f.popEstimate2016 = 0 or f.popEstimate2016 is null then 0 else f.tractlowi/f.popEstimate2016*100 end as low_inc_pop_per, case when f.ohu2010 = 0 or f.ohu2010 is null then 0 else f.tractsnap/f.ohu2010*100 end as snap_households_per from population p left join obeseDiabetes o on p.fips = o.fips left join foodDesert f on p.fips = f.fips left join countyMcdsCount m on p.fips = m.county_fips;"
...
select p.fips, p.stname,p.ctyname, p.abbrev,p.popestimate2016, 
	...
	o.obesity_n, o.obesity_per, 
	case when o.diab_per is null or o.diab_per = 0 then 0 else o.diab_n/(o.diab_per/100) end as diab_pop,
	o.diab_n, o.diab_per, 
	f.pop2010, f.ohu2010, f.medianfamilyincome, f.lapop20, f.lalowi20, f.lasnap20, 
	...
	case when f.popEstimate2016 = 0 or f.popEstimate2016 is null then 0 else f.tractlowi/f.popEstimate2016*100 end as low_inc_pop_per,
	case when f.ohu2010 = 0 or f.ohu2010 is null then 0 else f.tractsnap/f.ohu2010*100 end as snap_households_per
	from population p 
