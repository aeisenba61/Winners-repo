...
,f.lapop20\
,f.lalowi20, f.lasnap20, f.tractlowi, f.tractsnap, f.lapop20_share, f.lalowi20_share, f.lasnap20_share, m.mccount\
,case when m.mccount is null or p.popestimate2016 = 0 then 0 else m.mccount/p.popestimate2016 end as mcd_per_cap\
,case when p.popEstimate2016 = 0 or p.popEstimate2016 is null then 0 else f.tractlowi/p.popEstimate2016*100 end as low_inc_pop_per \
,case when f.ohu2010 = 0 or f.ohu2010 is null then 0 else f.tractsnap/f.ohu2010*100 end as snap_households_per \
from population p left join obeseDiabetes o on p.fips = o.fips \
