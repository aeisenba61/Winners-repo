...
// [obesity_per, diab_per, mcd_per_cap, low_inc_pop_per, snap_households_per]
var castColumnsToNumber = true;
// ------------------ END SETTINGS

csv().from.path(__dirname + '/countyIn.csv', {
	delimiter: ',',
	escape: '"'
	...
	fs.readFile(__dirname + '/in.geojson', 'utf8', function(err, data) {
		...
		fs.writeFile(__dirname + '/countyOut.geojson', JSON.stringify(joinedData, null, 4), function(err) {
			if (err) {
				console.log(err);
			} else {
